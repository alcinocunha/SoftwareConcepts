module Apps/WOPR/NoSecretsInTrash2
open Action
open Reaction
...
check Invariant {
	always {
		no Reaction iff {
			no Secret & trashed
			all f : File | f in accessible iff before (not T.delete[f] since (T.create[f] or (T.restore[f] and no Secret & trashed)))
		} 
	}
...
run Scenario3 {
	eventually (DeleteEmpty and some f : File-Secret | T.delete[f])
	eventually always no Reaction		
} for exactly 3 File, 4 Action, 3 Reaction expect 0
...
when
	T.delete[f]
require
	t not in Secret implies no Secret & trashed
*/

fact {
	all f : File | always {
		T.delete[f] implies (f not in Secret implies no Secret & trashed)
	}
}
