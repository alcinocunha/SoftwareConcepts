...
| [Reservation](Concepts/Reservation.als) | `User` `Resource` | `available ⊆ Resource` `reservations ⊆ User × Resource` | `provide` `retract` `reserve` `cancel` `use` | [Restaurant1](Apps/Restaurant1.als) |
...
| [Restaurant2](Apps/Restaurant2.als) | [Email](Concepts/Email.als) [Reservation](Concepts/Reservation.als) | A restaurant that receives reservation requests via email. Still buggy because a used table is undistinguishable from an available one. |
